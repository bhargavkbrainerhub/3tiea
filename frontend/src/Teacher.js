...
function Teacher() {
  ...
  const [data, setData] = useState([]); // This will hold the array of teacher objects
  ...
  const getData = () => {
    console.log(`Fetching data from: ${API_BASE_URL}/teacher`); // Log the URL being called
    fetch(`${API_BASE_URL}/teacher`)
      .then((res) => {
        console.log('Response received:', res); // Log the raw response object
        if (!res.ok) {
          ...
          <tbody>
            {data.map((d, i) => (
              <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#f9f9f9' : '#fff' }}>
                ...
                <td style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'center' }}>{d.subject}</td>
